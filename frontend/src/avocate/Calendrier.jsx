...
import { getAppointments, getTrials, getAudiences, createAppointment, deleteAppointment, deleteAudience, getUsersByRole } from '../api';
import { Plus, Search, SlidersHorizontal, X, CalendarCheck, Scale, RefreshCw, Calendar } from 'lucide-react';
...
const Calendrier = () => {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'events', 'meetings', 'reminders'
    ...
    Promise.all([getAppointments(), getTrials(), getAudiences()])
      .then(([apptRes, trialRes, audRes]) => {
        const appts = apptRes.data
          .filter(a => a.status === 'CONFIRMED' && a.appointmentDate)
          .map(a => ({
            id: a.ida,
        ...
        const trials = trialRes.data.map(t => ({
          id: `t_${t.idt}`,
  ...
  const handleDelete = (eventId) => {
    ...
    } else if (idStr.startsWith('t_')) {
      if (!window.confirm('Supprimer cette audience ?')) return;
      // trials deletion not implemented — skip
    } else {
  ...
  const getCategoryIcon = (category) => {
    switch (category) {
      case 'appointment':  return <CalendarCheck size={13} />;
      case 'hearing':      return <Scale size={13} />;
      case 'postponed':    return <RefreshCw size={13} />;
      case 'consultation': return <CalendarCheck size={13} />;
  ...
  const getEventsForDate = (dateStr) => {
    return filteredEvents.filter(event => event.date_time.split(' ')[0] === dateStr);
  };
      ...
      <div className="week-controls">
        <div className="week-nav-left">
          ...
          <div className="arrow-group">
            <button className="nav-arrow" onClick={() => navigate(-1)} title="Précédent">
              &#8249;
            </button>
            <button className="nav-arrow" onClick={() => navigate(1)} title="Suivant">
              &#8250;
            </button>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowNewModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowNewModal(false)}><X size={16} /></button>
            </div>
            <form onSubmit={(e) => {
              e.preventDefault();
              ...
              return (
                <div key={timeIndex} className="time-row">
                  ...
                  {daysOfWeek.map((day, dayIndex) => {
                    const dayEvents = getEventsForDayAndTime(day.dateStr, hour);
                    ...
                    return (
                      <div key={dayIndex} className={`time-slot ${day.isToday ? 'today-col' : ''}`}>
                        {dayEvents.map((event) => (
                          <div
                            key={event.id}
                            className={`event-block ${event.event_category}`}
                          >
                            <div className="event-time">
                              {formatEventRange(event.date_time, event.duration)}
                            </div>
        ...
        <div className="schedule-container day-view">
          <div className="day-view-grid">
            {timeSlots.map((slot, timeIndex) => {
              ...
              const dayEvents = getEventsForDayAndTime(dayData.dateStr, hour);
