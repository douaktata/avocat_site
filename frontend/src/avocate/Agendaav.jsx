...
// Normalize backend DTO fields to frontend names
const withSlots = (s) => {
  const date = s.date || s.validFrom || '';
  const heureDebut = s.heureDebut || (s.startTime ? s.startTime.substring(0, 5) : '');
  const heureFin = s.heureFin || (s.endTime ? s.endTime.substring(0, 5) : '');
  const dureeConsultation = s.dureeConsultation || s.defaultDuration || 30;
  const actif = s.actif !== undefined ? s.actif : (s.active !== undefined ? s.active : true);
  return {
    ...s,
    date,
    heureDebut,
    ...
    dureeConsultation,
    actif,
    creneauxGeneres: generateTimeSlots(heureDebut, heureFin, dureeConsultation),
  };
};
...
const Agendaav = () => {
  ...
  // Grouper les créneaux par leur date spécifique
  ...
  const handleToggleSlot = (id) => {
    ...
    updateSlot(id, {
      validFrom: updated.date,
      validUntil: updated.date,
      startTime: updated.heureDebut,
      endTime: updated.heureFin,
      defaultDuration: updated.dureeConsultation,
  ...
  const handleAddSlot = (e) => {
    e.preventDefault();
    const payload = {
      validFrom: newSlot.date,
      validUntil: newSlot.date,
      startTime: newSlot.heureDebut,
      endTime: newSlot.heureFin,
      defaultDuration: newSlot.dureeConsultation,
      active: newSlot.actif,
      avocatId,
    };
    createSlot(payload)
      .then(res => {
      ...
      <div className="agenda-stats">
        <div className="stat-box">
          ...
          <div className="stat-info">
            <h3>{stats.creneauxTotal}</h3>
            <p>Créneaux configurés</p>
          </div>
        </div>
        <div className="stat-box">
        ...
        <div className="stat-box">
          ...
          <div className="stat-info">
            <h3>{stats.creneauxDisponibles}</h3>
            <p>Plages réservables</p>
          </div>
        </div>
        <div className="stat-box">
